...
def training_loop(
    ...
    # Initialize.
    cpu_gpu = 1 if num_gpus == 0 else num_gpus
    start_time = time.time()
    device = torch.device(
        'cpu') if num_gpus == 0 else torch.device('cuda', rank)
    np.random.seed(random_seed * cpu_gpu + rank)
    torch.manual_seed(random_seed * cpu_gpu + rank)
    ...
    training_set_sampler = misc.InfiniteSampler(
        dataset=training_set, rank=rank, num_replicas=cpu_gpu, seed=random_seed)
    training_set_iterator = iter(torch.utils.data.DataLoader(
        dataset=training_set, sampler=training_set_sampler, batch_size=batch_size//cpu_gpu, **data_loader_kwargs))
    if rank == 0:
        print()
    ...
    if rank == 0:
        print('Printing network summary tables')
    ...
    if rank == 0:
        print(f'Distributing across {cpu_gpu} GPUs...')
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            ...
            all_gen_c = torch.from_numpy(
                np.stack(all_gen_c)).pin_memory().to(device) if num_gpus > 0 else torch.from_numpy(
                np.stack(all_gen_c)).to(device)
        ...
        if (snapshot_data is not None) and (len(metrics) > 0):
            ...
            for metric in metrics:
                result_dict = metric_main.calc_metric(metric=metric, G=snapshot_data['G_ema'],
                                                      dataset_kwargs=training_set_kwargs, num_gpus=cpu_gpu, rank=rank, device=device)
                if rank == 0:
...
def get_elapsed_time(prof):
    lines = prof.key_averages().table().splitlines()
    n_lines = len(lines)
    last_line = lines[n_lines - 1]
    # miliseconds_f = last_line.split(": ")[1]
    # miliseconds = miliseconds_f.split("ms")[0].split("s")[0]
    # To Do: take on consideration seconds and miliseconds
    return float(re.findall('[0-9]+.[0-9]+', last_line)[0])
